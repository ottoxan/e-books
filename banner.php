<section class="hero">
    ...
    <div class="text">
        <h1><span class="highlight"><?= __('Books For Everyone') ?></span></h1>
        <p><?= __('Access it from anywhere, anytime. Let\'s read eBooks!') ?></p>

        <div class="search-box">
            <input type="text" id="searchInputBanner" placeholder="<?= __('Search a Book') ?>" />
            <button id="searchBtn"><?= __('Search') ?></button>
        </div>
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                document.getElementById('searchBtn').onclick = function() {
                    var query = document.getElementById('searchInputBanner').value;
                    if (query.trim() !== "") {
                        window.location.href = 'bookSearch.php?q=' + encodeURIComponent(query);
                    }
                };
                document.getElementById('searchInputBanner').addEventListener('keydown', function(event) {
                    if (event.key === 'Enter') {
