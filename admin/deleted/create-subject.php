...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($subject)) {
        ...
        $sql = "INSERT INTO subject (semester_id, subject) VALUES (?, ?)";
        $stmt = $mysqli->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("is", $semester_id, $subject);
            if ($stmt->execute()) {
            ...
            <div class="form-group">
                ...
                <select class="form-select mb-3" id="semester_id" name="semester_id" aria-label="Default select example">
                    ...
                    <?php while ($row = $resultSemesters->fetch_assoc()): ?>
                        <option value="<?php echo htmlspecialchars($row["semester_id"]); ?>">
                            <?php echo htmlspecialchars($row["semester_number"] . " - " . $row["grade"] . " (" . $row["academic_stage"] . ")"); ?>
                        </option>
                    <?php endwhile; ?>
                </select>
            </div>
